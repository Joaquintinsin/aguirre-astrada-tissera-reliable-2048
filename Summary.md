...
Para dar cierre al resumen, agregamos que como estamos frente a una metodogia agil incremental, que es lo usado hoy en dia en el desarrollo de aplicaciones, se agregaron nuevos tests incluso estando en la fase 4, que resultan en una mejor test-suite, y este se refleja con los ultimos commits en el repositorio. Como complemento técnico se integró JaCoCo para medir de forma precisa la calidad del código. Logrando una cobertura del 77% en ramas y sentencias (obviando la clase main, ya que contenía principalmente impresiones por consola, y métodos estándar como hash o clone).
